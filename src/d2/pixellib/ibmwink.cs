...
public unsafe partial struct IBITMAP
{
    ...
    delegate int iBitmapUpdate(int x, int y, int w, uint* card, void* user);

    public static void ibitmap_fetch_8(in IBITMAP src, byte* card8, int line)
    {
        byte mask;
        int overflow;
        int size;
        ...
        if (ibitmap_pixfmt_guess(src) == IPIX_FMT_G8)
        {
            ...
            mask = (byte)_ipixel_to_gray(r, g, b);
        }
        else
        {
            uint r, g, b, a;
        ...
        switch (overflow)
        {
            ...
            case IBOM_REPEAT:
                ...
                card8[0] = ((byte*)src.line[line])[0];
                card8[size - 1] = ((byte*)src.line[line])[(int)src.w - 1];
                break;
            case IBOM_WRAP:
                line = line % h;
                if (line < 0) line += h;
                card8[size - 1] = ((byte*)src.line[line])[0];
                card8[0] = ((byte*)src.line[line])[(int)src.w - 1];
                break;
            case IBOM_MIRROR:
    ...
    public static void ibitmap_fetch_32(in IBITMAP src, uint* card, int line)
    {
        iColorIndex* index = (const iColorIndex*)src.extra;
        iFetchProc fetch;
        uint mask;
        int overflow;
        int size;
    ...
    public static inline byte ibitmap_filter_pass_8( byte* pixel, short* filter)
{
...
public static inline uint
ibitmap_filter_pass_32(const uint* pixel, short* filter)
{
...
int ibitmap_filter_8(ref IBITMAP dst, in IBITMAP src, short* filter)
{
    byte* buffer, *p1, *p2, *p3, *p4, *card;
    byte pixel[9];
    int line, i;

    buffer = (byte*)malloc((src.w + 2) * 3);
...
int ibitmap_filter_32(ref IBITMAP dst, in IBITMAP src, short* filter)
{
    iColorIndex* index = (iColorIndex*)dst.extra;
    uint* buffer, *p1, *p2, *p3, *p4, *card;
    uint pixel[9];
    iStoreProc store;
    int line, i;

    buffer = (uint*)malloc((src.w + 2) * 4 * 4);
    if (buffer == null) return -10;
...
//---------------------------------------------------------------------
int ibitmap_filter(ref IBITMAP dst, short* filter)
{
    IBITMAP* src;
    int retval;
    int pixfmt;
...
//---------------------------------------------------------------------
int ibitmap_channel_get(ref IBITMAP dst, int dx, int dy, in IBITMAP src,
    int sx, int sy, int sw, int sh, int channel)
{
	...
	public static int table[4] = { 1, 2, 3, 0 };
#else
    public static int table[4] = { 2, 1, 0, 3 };
#endif
    ...
    if (pixfmt == IPIX_FMT_A8R8G8B8)
    {
        ...
        for (y = 0; y < sh; y++)
        {
            byte* ss = (byte*)(src.line[sy + y]) + 4 * sx;
            byte* dd = (byte*)(dst.line[dy + y]) + 1 * dx;
...
//---------------------------------------------------------------------
int ibitmap_channel_set(ref IBITMAP dst, int dx, int dy, in IBITMAP src,
    int sx, int sy, int sw, int sh, int channel)
{
    byte _buffer[IBITMAP_STACK_BUFFER];
    byte* buffer = _buffer;
    ...
    if (pixfmt == IPIX_FMT_A8R8G8B8)
    {
        ...
        for (y = 0; y < sh; y++)
        {
            byte* ss = (byte*)(src.line[sy + y]) + 1 * sx;
            byte* dd = (byte*)(dst.line[dy + y]) + 4 * dx;
...
int ibitmap_update(IBITMAP* dst, ref IRECT bound,
    iBitmapUpdate updater, int readonly, void* user);

//---------------------------------------------------------------------
// 图像更新：按照从上到下顺序，每条扫描线调用一次updater
//---------------------------------------------------------------------
int ibitmap_update(ref IBITMAP dst, ref IRECT bound,
    iBitmapUpdate updater, int readonly, void* user)
{
    byte _buffer[IBITMAP_STACK_BUFFER];
    byte* buffer = _buffer;
...
// 基础特效
//=====================================================================
IBITMAP* ibitmap_effect_drop_shadow(in IBITMAP src, int dir, int level)
{
    public static short f0[9] = { 16, 16, 16, 16, 128, 16, 16, 16, 16 };
    IBITMAP* alpha;
    short* filter;
    int i;
...
public static int[] g_stack_blur8_mul = [
...
public static   int[] g_stack_blur8_shr = [
...
void ipixel_stackblur_4(void* src, int pitch, int w, int h, int rx, int ry)
{
    ...
    if (rx > 0)
    {
        ...
        for (y = 0; y < (uint)h; y++)
        {
            ...
            src_pix_ptr = (byte*)src + y * pitch;

            for (i = 0; i <= (uint)rx; i++)
            {
                stack_pix_ptr = (byte*)&stack[i];
            ...
            src_pix_ptr = (byte*)src + y * pitch + xp * 4;
            dst_pix_ptr = (byte*)src + y * pitch;

            for (x = 0; x < (uint)w; x++)
            {
                dst_pix_ptr[0] = (byte)((sum_r * mul_sum) >> shr_sum);
                dst_pix_ptr[1] = (byte)((sum_g * mul_sum) >> shr_sum);
                dst_pix_ptr[2] = (byte)((sum_b * mul_sum) >> shr_sum);
                dst_pix_ptr[3] = (byte)((sum_a * mul_sum) >> shr_sum);
                dst_pix_ptr += 4;
                ...
                stack_pix_ptr = (byte*)&stack[stack_start];
                ...
                stack_pix_ptr = (byte*)&stack[stack_ptr];
    ...
    if (ry > 0)
    {
        ...
        for (x = 0; x < (uint)w; x++)
        {
            ...
            src_pix_ptr = (byte*)src + yp * pitch + x * 4;
            dst_pix_ptr = (byte*)src + x * 4;
...
// 图像模糊
void ibitmap_stackblur(IBITMAP* src, int rx, int ry, ref IRECT bound)
{
    int x, y, w, h;
    IRECT rect;
    ...
    if (src.bpp != 32)
    {
        ...
        ipixel_stackblur_4(newbmp.pixel, (int)newbmp.pitch,
            w, h, rx, ry);
    ...
    ipixel_stackblur_4((byte*)src.line[y] + x * 4, (int)src.pitch,
        w, h, rx, ry);
}

// 生成阴影
IBITMAP* ibitmap_drop_shadow(in IBITMAP src, int rx, int ry)
{
    IBITMAP* alpha, *beta;
    alpha = ibitmap_create((int)src.w, (int)src.h, 8);
    if (alpha == null) return null;
...
// 生成圆角
IBITMAP* ibitmap_round_rect(in IBITMAP src, int radius, int style)
{
    IBITMAP* alpha, *beta;
    int w = (int)src.w;
    int h = (int)src.h;
    ...
    for (j = 0; j < h4; j++)
    {
        byte* srcpix = (byte*)alpha.line[j];
        byte* dstpix = (byte*)beta.line[j];
    ...
    else if (style == 1 || style == 2)
    {
        ...
        for (j = 0; j < h4; j++)
        {
            byte* srcpix = (byte*)alpha.line[j];
            byte* dstpix = (byte*)beta.line[j];
            uint limit = (style == 1) ? (h4 / 2) : (h4);
            long pos = ((long)j * 0xffff) / limit;
            uint color = ipixel_gradient_walker_pixel(&walker, pos);
            uint cc, r1, g1, b1, a1, r2, g2, b2, a2, save;
            ...
            for (i = w4; i > 0; i--, srcpix += 4, dstpix += 4)
            {
                if (srcpix[_ipixel_card_alpha] && dstpix[_ipixel_card_alpha])
                {
                    ...
                    *(uint*)dstpix = IRGBA_TO_A8R8G8B8(r2, g2, b2, save);
                }
            }
        }
    }
    else if (style == 3)
    {
        ...
        card = (uint*)alpha.pixel;

        for (j = 0; j < h4; j++)
        {
            byte* dstpix = (byte*)beta.line[j];
            byte* srcpix = (byte*)card;
            uint c1, c2, r1, g1, b1, a1, r2, g2, b2, a2;
            ...
            for (i = w4; i > 0; srcpix += 4, dstpix += 4, i--)
            {
                if (dstpix[_ipixel_card_alpha])
                {
                    ...
                    IBLEND_ADDITIVE(r1, g1, b1, a1, r2, g2, b2, a2);
                    *(uint*)dstpix = IRGBA_TO_A8R8G8B8(r2, g2, b2, a2);
                }
            }
        }
    }
...
public static int i_update_trans(int x, int y, int w, uint* card, void* user)
{
...
public static int i_update_add(int x, int y, int w, uint* card, void* user)
{
    uint cc = *(uint*)user;
    int cr, cg, cb, ca;
    int c, r, g, b, a;
...
public static int i_update_sub(int x, int y, int w, uint* card, void* user)
{
    uint cc = *(uint*)user;
    int cr, cg, cb, ca;
    int c, r, g, b, a;
...
public static int i_update_mul(int x, int y, int w, uint* card, void* user)
{
    uint cc = *(uint*)user;
    int cr, cg, cb, ca;
    int c, r, g, b, a;
...
// 色彩变幻：输入 5 x 5矩阵
void ibitmap_color_transform(ref IBITMAP dst, ref IRECT b, float* t)
{
    cfixed transform[20];
    int i;
    for (i = 0; i < 20; i += 4)
...
// 色彩变幻：加法
void ibitmap_color_add(ref IBITMAP dst, ref IRECT b, uint color)
{
    ibitmap_update(dst, b, i_update_add, 0, &color);
}

// 色彩变幻：减法
void ibitmap_color_sub(ref IBITMAP dst, ref IRECT b, uint color)
{
    ibitmap_update(dst, b, i_update_sub, 0, &color);
}

// 色彩变幻：乘法
void ibitmap_color_mul(ref IBITMAP dst, ref IRECT b, uint color)
{
...
public static inline int _iencode(int x, int y, int left, int top, int right,
    int bottom)
{
    int code = 0;
...
public static inline int _iencode_float(float x, float y, int left, int top,
...
public static inline int _iclipline(int* pts, int left, int top, int right,
    int bottom)
{
    int x1 = pts[0];
    int y1 = pts[1];
...
#define ipaint_get_count(paint, base) \
    (((int)((int*)paint - (int*)base)) >> 1)


int ibitmap_put_line_low(ref IBITMAP dst, int x1, int y1, int x2, int y2,
    uint color, int additive, ref IRECT clip, void* workmem)
{
    uint* paint;
    int cl, ct, cr, cb;
    int x, y, p, n, tn;
    ...
    if (workmem == null)
    {
        ...
        int ds1 = (dst.w + dst.h);
        int ds2 = (dx + dy);
        int size = (ds1 < ds2) ? ds1 : ds2;
        return (size + 2) * 4 * sizeof(uint);
    }
    ...
    count = (int)(paint - (uint*)workmem) / 2;

    ibitmap_draw_pixel_list_sc(dst, (uint*)workmem, count, color,
        additive);
...
// 无AA绘制直线
int ibitmap_put_line(ref IBITMAP dst, int x1, int y1, int x2, int y2,
    uint color, int additive, ref IRECT clip)
{
    byte _buffer[IBITMAP_STACK_BUFFER];
    byte* buffer = _buffer;
    int dx = (x1 < x2) ? (x2 - x1) : (x1 - x2);
...
void ibitmap_put_circle_low(ref IBITMAP dst, int x0, int y0, int r, int fill,
	const ref IRECT clip, uint color, int additive, void* workmem, int size)
{
    int* buffer = (int*)workmem;
    int* paint = buffer;
    int cx = 0, cy = r;
...
#define ipaint_flush_limit(maxcount) do { \
    int count = ipaint_get_count(paint, buffer); \
			if (count > maxcount)
{ \
				ibitmap_draw_pixel_list_sc(dst, (uint*)buffer, \
...
{ \
    ...
    // 无AA绘制圆形
void ibitmap_put_circle(ref IBITMAP dst, int x0, int y0, int r, int fill,
	const ref IRECT clip, uint color, int additive)
{
    byte buffer[IBITMAP_STACK_BUFFER];
...
// 初始化缓存
void cvector_init(ref CVECTOR vector)
{
    vector.data = null;
    vector.size = 0;
    vector.block = 0;
...
// 销毁缓存
void cvector_destroy(ref CVECTOR vector)
{
...
// 改变缓存大小
int cvector_resize(ref CVECTOR vector, size_t size)
{
    byte* lptr;
    size_t block, min;
    size_t nblock;
    ...
    for (nblock = sizeof(byte*); nblock < size;) nblock <<= 1;
    block = nblock;
...
// 添加数据到末尾
int cvector_push(ref CVECTOR vector, void* data, size_t size)
{
...
// 原始作图
//---------------------------------------------------------------------
public static public struct CVECTOR ipixel_scratch = { 0, 0, 0 };

// 批量填充梯形
int ipixel_render_traps(ref IBITMAP dst, ipixel_trapezoid_t* traps,
    int ntraps, IBITMAP* alpha, ipixel_source_t* src, int isadd,
	const ref IRECT clip, ref CVECTOR scratch)
{
    ...
    {
        spans = (ipixel_span_t*)(scratch.data + sizeof(uint) * cw);
        card = (uint*)scratch.data;
        color = 0;
    }
    ...
    {
        int y = spans[i].y;
        byte* mask = (byte*)alpha.line[y] + spans[i].x;
...
// 绘制多边形
int ipixel_render_polygon(ref IBITMAP dst, ipixel_point_fixed_t* pts,
    int npts, IBITMAP* alpha, ipixel_source_t* src, int isadditive,
	const ref IRECT clip, ref CVECTOR scratch)
{
    byte _buffer[IBITMAP_STACK_BUFFER];
    byte* buffer = _buffer;
...
// 作图接口
//---------------------------------------------------------------------
public static void ipaint_init(ipaint_t* paint)
{
    paint.image = null;
    paint.alpha = null;
    paint.additive = 0;
...
public static inline int ipaint_point_push(ipaint_t* paint, double x, double y)
{
    int size = sizeof(ipixel_point_t) * (paint.npts + 1);
    if (size > (int)paint.points.size)
    {
...
int ipaint_point_append(ipaint_t* paint, ipixel_point_t* pts, int n)
{
    int size = sizeof(ipixel_point_t) * (n + paint.npts);
    if (size > (int)paint.points.size)
    {
        if (cvector_resize(&paint.points, size) != 0)
            return -1;
...
public static int ipaint_point_convert(ipaint_t* paint)
{
...
public static ipixel_point_fixed_t* ipaint_point_to_fixed(ipaint_t* paint)
{
...
// 设置当前颜色
void ipaint_set_color(ipaint_t* paint, uint color)
{
    paint.color = color;
...
// 设置裁剪窗口
void ipaint_set_clip(ipaint_t* paint, ref IRECT clip)
{
    if (clip != null)
    {
        IRECT size;
...
// 设置字体颜色
void ipaint_text_color(ipaint_t* paint, uint color)
{
    paint.text_color = color;
}

// 设置字体背景
void ipaint_text_background(ipaint_t* paint, uint color)
{
...
// 绘制多边形
int ipaint_draw_polygon(ipaint_t* paint, ipixel_point_t* pts, int n)
{
    ...
    double y2)
{
    ...
    else
    {
        ...
        if (size > (int)paint.scratch.size)
        {
...
void ipaint_fill(ipaint_t* paint, ref IRECT rect, uint color)
{
    IRECT box;
    if (rect == null)
    {
...
void ipaint_cprintf(ipaint_t* paint, int x, int y, byte* fmt, ...)
{
    byte buffer[4096];
    va_list argptr;
    va_start(argptr, fmt);
...
void ipaint_sprintf(ipaint_t* paint, int x, int y, byte* fmt, ...)
{
    byte buffer[4096];
    va_list argptr;
    va_start(argptr, fmt);
...
int ipaint_raster(ipaint_t* paint, ipixel_point_t* pts,
	const IBITMAP* image, ref IRECT rect, uint color, int flag)
{
...
int ipaint_raster_draw(ipaint_t* paint, double x, double y, in IBITMAP src,
	const ref IRECT rect, double off_x, double off_y, double scale_x,
    double scale_y, double angle, uint color)
{
...
int ipaint_raster_draw_3d(ipaint_t* paint, double x, double y, double z,
    in IBITMAP src, ref IRECT rect, double off_x, double off_y,
    double scale_x, double scale_y, double angle_x, double angle_y,
    double angle_z, uint color)
{
...
int ipaint_draw(ipaint_t* paint, int x, int y, in IBITMAP src,
	const ref IRECT bound, uint color, int flags)
{
    int sx, sy, sw, sh;
    if (bound == null)
    {
...
void iconv_RGB_to_YCrCb(const IRGB* input, int size, IYCrCb* output)
{
...
void iconv_YCrCb_to_RGB(const IYCrCb* input, int size, IRGB* output)
{
...
void iconv_RGB_to_HSV(const IRGB* input, int size, IHSV* output)
{
...
void iconv_HSV_to_RGB(const IHSV* input, int size, IRGB* output)
{
    ...
    for (; size > 0; size--, input++, output++)
    {
        ...
        output.r = (byte)(r * 255.0f);
        output.g = (byte)(g * 255.0f);
        output.b = (byte)(b * 255.0f);
        output.reserved = input.reserved;
    }
}
...
// 生成特效
public static IBITMAP* ibitmap_glossy_make(this ref IBITMAP bmp, int radius, int border, int light,
...
// 更新 HSV
public static int ibitmap_update_hsv(int x, int y, int w, uint* card, void* user)
{
    float h = ((IHSV*)user).H;
    float s = ((IHSV*)user).S;
    float v = ((IHSV*)user).V;
...
// 更新 HSL
public static int ibitmap_update_hsl(int x, int y, int w, uint* card, void* user)
{
    float h = ((IHSV*)user).H;
    float s = ((IHSV*)user).S;
    float l = ((IHSV*)user).V;
...
// 调整色调
void ibitmap_adjust_hsv(ref IBITMAP bmp, float hue, float saturation,
    float value, ref IRECT bound)
{
    IHSV hsv;
    hsv.H = (float)hue;
...
// 调整色调
void ibitmap_adjust_hsl(ref IBITMAP bmp, float hue, float saturation,
    float lightness, ref IRECT bound)
{
    IHSV hsv;
    hsv.H = (float)hue;
    hsv.S = (float)saturation;
...
// 计算新大小下面的 Patch9
int ipixel_patch_nine(int* patch, int count, int newsize, int*out)
{
...
#else
            int newlen = (int)((((int int)oldlen) *dstlen) / srclen);
#endif
    if (k == nblack - 1)
    {
        int j, z;
...
// 扫描 Patch
int ipixel_patch_scan(void* src, int step, int size, int* patch)
{
    byte* ptr = (byte*)src;
    int index = 0, mode = 0, i = 0, count = 0;
    uint endian = 0x11223344;
    int lsb = (*((byte*)&endian) == 0x44) ? 1 : 0;
    int A = lsb ? 3 : 0;
    int R = lsb ? 2 : 1;
    int G = lsb ? 1 : 2;
    int B = lsb ? 0 : 3;
    ...
    {
        uint a1 = ptr[A] + (ptr[A] >> 7);
        uint a2 = 256 - a1;
        uint cr = (ptr[R] * a1 + 255 * a2) >> 8;
        uint cg = (ptr[G] * a1 + 255 * a2) >> 8;
        uint cb = (ptr[B] * a1 + 255 * a2) >> 8;
        uint cc = (cr + cg + cg + cb) >> 2;
...
// 位图 patch 扫描
int ibitmap_patch_scan(in IBITMAP src, int n, int* patch)
{
    byte* ptr = (byte*)src.pixel;
    int pitch = (int)src.pitch;
    int w = (int)src.w;
    int h = (int)src.h;
    int count;
...
// 平滑缩放
int ibitmap_smooth_resize(ref IBITMAP dst, ref IRECT rectdst,
    in IBITMAP src, ref IRECT rectsrc);

// Android: patch 9 生成
// 位图 patch
IBITMAP* ibitmap_patch_nine_i(in IBITMAP src, int nw, int nh, int* code)
{
    int countx, county;
    int* srcx, *srcy;
    int* dstx, *dsty;
...
// 位图 patch
IBITMAP* ibitmap_patch_nine(in IBITMAP src, int nw, int nh, int* code)
{
    IBITMAP* tmp, *pp;
    int sw = (int)src.w;
    int sh = (int)src.h;
...
// Android: 取得客户区
// 取得客户区
int ibitmap_patch_client(in IBITMAP src, ref IRECT client)
{
    int _buffer[1024];
    int* buffer = _buffer;
    int n3, n4, nn;
