...
// 返回   -2 锁定时候碰到的其他错误 
int DDrawSurfaceLock(CSURFACE *dst, void **bits, int *pitch, int flags);
...
// 设置关键色
int DDrawSurfaceSetMask(CSURFACE *dst, uint mask);
...
CSURFACE* DDrawCreateFromDesc(const DDSURFACEDESC2* desc, HRESULT* hr)
{
	...
	surface.bits = (byte*)ddsd.lpSurface;
...
int DDrawSurfaceLock(CSURFACE* dst, void** bits, int* pitch, int flags)
{
	...
	dst.lock = 1;
	dst.bits = (byte*)ddsd.lpSurface;
	dst.pitch = (int)ddsd.dwLinearSize;

	if (bits) *bits = (void*)ddsd.lpSurface;
	if (pitch) *pitch = (int)ddsd.dwLinearSize;
