...
public struct IMDIO
{
    public public static delegate*<ref IMDIO, int> getc;
    public public static delegate*<ref IMDIO, int, int> putc;
    int _code, _pos, _len, _ch, _bitc, _bitd;
    int _cnt, _ungetc;
    void* data;
}

int is_getc(ref IMDIO stream);
int is_putc(ref IMDIO stream, int c);

int is_ungetc(ref IMDIO stream, int c);

int is_reader(ref IMDIO stream, void* buffer, int size);
int is_writer(ref IMDIO stream, void* buffer, int size);

int is_igetw(ref IMDIO stream);
int is_iputw(ref IMDIO stream, int w);
int is_igetl(ref IMDIO stream);
int is_iputl(ref IMDIO stream, int l);

int is_mgetw(ref IMDIO stream);
int is_mputw(ref IMDIO stream, int w);
int is_mgetl(ref IMDIO stream);
int is_mputl(ref IMDIO stream, int l);

int is_igetbits(ref IMDIO stream, int count);
int is_iputbits(ref IMDIO stream, int data, int count);
void is_iflushbits(ref IMDIO stream, int isflushwrite);

int is_mgetbits(ref IMDIO stream, int count);
int is_mputbits(ref IMDIO stream, int data, int count);
void is_mflushbits(ref IMDIO stream, int isflushwrite);

void is_seekcur(ref IMDIO stream, int skip);


//---------------------------------------------------------------------
// stream operation
//---------------------------------------------------------------------
int is_init_mem(ref IMDIO stream, void* lptr, int size);
int is_open_file(ref IMDIO stream, byte* filename, byte* rw);
int is_close_file(ref IMDIO stream);

void _is_perrno_set(int v);
int _is_perrno_get(void);

void* is_read_marked_block(ref IMDIO stream, byte* mark,
...
public  IBITMAP * iload_bmp_stream(ref IMDIO stream, IRGB* pal);

// load bmp picture from stream
public  IBITMAP * iload_tga_stream(ref IMDIO stream, IRGB* pal);

// load single gif frame from stream
public  IBITMAP * iload_gif_stream(ref IMDIO stream, IRGB* pal);

// save bmp picture to stream
int isave_bmp_stream(ref IMDIO stream, in IBITMAP bmp, IRGB* pal);

// save tga picture to stream
int isave_tga_stream(ref IMDIO stream, in IBITMAP bmp, IRGB* pal);

// save gif picture to stream
int isave_gif_stream(ref IMDIO stream, in IBITMAP bmp, IRGB* pal);
...
// Add new loader to load other picture formats
 public delegate IBITMAP * IPICLOADER(ref IMDIO stream, IRGB* pal);
...
IPICLOADER ipic_loader(byte firstbyte, IPICLOADER loader);

// recognize data stream and load picture
public  IBITMAP * iload_picture(ref IMDIO stream, IRGB* pal);
...
int isave_bmp_file(byte* file, in IBITMAP bmp, IRGB* pal);

// save tga picture to file
int isave_tga_file(byte* file, in IBITMAP bmp, IRGB* pal);

// save gif picture to file
int isave_gif_file(byte* file, in IBITMAP bmp, IRGB* pal);
...
//---------------------------------------------------------------------
public struct IGIFDESC			/* gif descriptor */
{
    ...
    int delay;         /* extension - time delay */
    int frame;         /* extension - frame count */
    int hotx;           /* extension - hotpoint x */
    int hoty;           /* extension - hotpoint y */
    IRGB* pal;          /* current palette */
    IMDIO* stream;  /* input / output stream */
    public struct IBITMAP *bitmap;	/* bitmap */
	byte background;        /* background color */
    byte string[4096];		/* decode string */
	byte newc[4096];        /* dict new byte */
    public struct { short base, length; }
    str[4096];   /* lzw table */
};
...
int is_ungetc(IMDIO* stream, int c)
{
    ...
    stream._ungetc = (int)c;
    return c;
}

int is_reader(IMDIO* stream, void* buffer, int size)
{
    byte* lptr = (byte*)buffer;
    int total = 0;
    int ch;
...
int is_writer(IMDIO* stream, void* buffer, int size)
{
    byte* lptr = (byte*)buffer;
    int total = 0;
    int ch;
...
int is_igetl(IMDIO* stream)
{
    int c1, c2, c3, c4;
    if ((c1 = (int)is_getc(stream)) < 0) return -1;
    if ((c2 = (int)is_getc(stream)) < 0) return -1;
    if ((c3 = (int)is_getc(stream)) < 0) return -1;
    if ((c4 = (int)is_getc(stream)) < 0) return -1;
    return (c1 | (c2 << 8) | (c3 << 16) | (c4 << 24));
}

int is_iputl(IMDIO* stream, int l)
{
    int c1, c2, c3, c4;
    c1 = (l & 0x000000FF) >> 0;
    c2 = (l & 0x0000FF00) >> 8;
...
int is_mputl(IMDIO* stream, int l)
{
    int c1, c2, c3, c4;
    c1 = (l & 0xFF000000) >> 24;
    c2 = (l & 0x00FF0000) >> 16;
...
int is_getbits(IMDIO* stream, int count, int litend)
{
    int data, bitc, bitd, mask, c, i;
    bitc = stream._bitc;
    bitd = stream._bitd;
...
int is_putbits(IMDIO* stream, int data, int count, int litend)
{
    int bitc, bitd, mask, c;
    bitc = stream._bitc;
    bitd = stream._bitd;
    for (; count > 0;)
    {
...
int is_igetbits(IMDIO* stream, int count)
{
    return is_getbits(stream, count, 1);
}

int is_mgetbits(IMDIO* stream, int count)
{
    return is_getbits(stream, count, 0);
}

int is_iputbits(IMDIO* stream, int data, int count)
{
    return is_putbits(stream, data, count, 1);
}

int is_mputbits(IMDIO* stream, int data, int count)
{
...
void is_seekcur(IMDIO* stream, int skip)
{
...
//---------------------------------------------------------------------
#define _is_bmline(bmp, y) ((byte*)((bmp).line[y]))

public static inline uint _is_get3b(const void* p)
{
    return _ipixel_fetch_24(p, 0);
}

public static inline uint _is_put3b(void* p, uint c)
{
...
#define _IS_HEADER1() do { \
    ...
    public static inline uint _is_get8(in IBITMAP bmp, int x, int y)
{
    _IS_HEADER1();
    return _is_bmline(bmp, y)[x];
}

public static inline uint _is_get15(in IBITMAP bmp, int x, int y)
{
    _IS_HEADER1();
    return ((ushort*)_is_bmline(bmp, y))[x];
}

public static inline uint _is_get16(in IBITMAP bmp, int x, int y)
{
    _IS_HEADER1();
    return ((ushort*)_is_bmline(bmp, y))[x];
}

public static inline uint _is_get24(in IBITMAP bmp, int x, int y)
{
    _IS_HEADER1();
    return _is_get3b(_is_bmline(bmp, y) + (x) * 3);
}

public static inline uint _is_get32(in IBITMAP bmp, int x, int y)
{
    _IS_HEADER1();
    return ((uint*)_is_bmline(bmp, y))[x];
}

public static inline uint _is_put8(in IBITMAP bmp, int x, int y, uint c)
{
    _IS_HEADER1();
    _is_bmline(bmp, y)[x] = (byte)(c & 0xFF);
    return c;
}

public static inline uint _is_put15(in IBITMAP bmp, int x, int y, uint c)
{
    _IS_HEADER1();
    ((ushort*)_is_bmline(bmp, y))[x] = (ushort)(c & 0xFFFF);
    return c;
}

public static inline uint _is_put16(in IBITMAP bmp, int x, int y, uint c)
{
    _IS_HEADER1();
    ((ushort*)_is_bmline(bmp, y))[x] = (ushort)(c & 0xFFFF);
    return c;
...
public static inline uint _is_put24(in IBITMAP bmp, int x, int y, uint c)
{
    _IS_HEADER1();
...
public static inline uint _is_put32(in IBITMAP bmp, int x, int y, uint c)
{
    _IS_HEADER1();
    ((uint*)_is_bmline(bmp, y))[x] = c;
    return c;
}

public static inline uint _is_getpx(in IBITMAP bmp, int x, int y)
{
    _IS_HEADER1();
...
public static inline uint _is_putpx(in IBITMAP bmp, int x, int y, uint c)
{
    _IS_HEADER1();
...
int _ibitmap_guess_pixfmt(in IBITMAP bmp)
{
    int fmt;
...
uint _im_color_get(int fmt, uint c, IRGB* pal)
{
    int r, g, b, a;
...
void _is_perrno_set(int v) { _is_perrno = v; }
int _is_perrno_get(void) { return _is_perrno; }

//---------------------------------------------------------------------
// stream operation interface
//---------------------------------------------------------------------
#define IRWCHECK(s, v) do { if ((s)._code != (v)) return -1; } while(0)
public static int iproc_getc_mem(ref IMDIO stream)
{
    IRWCHECK(stream, 0);
    if (stream._pos < stream._len)
        return ((byte*)(stream.data))[stream._pos++];
    return -1;
}

public static int iproc_putc_mem(ref IMDIO stream, int c)
{
    byte ch = (byte)c;
    IRWCHECK(stream, 0);
    ...
    {
        ((byte*)(stream.data))[stream._pos++] = ch;
    }
    if (stream._pos > stream._len) stream._len = stream._pos;
    return c;
}

public static int iproc_getc_file(ref IMDIO stream)
{
    IRWCHECK(stream, 1);
    return fgetc((FILE*)(stream.data));
}

public static int iproc_putc_file(ref IMDIO stream, int c)
{
    IRWCHECK(stream, 1);
    return fputc(c, (FILE*)(stream.data));
}

int is_init_mem(IMDIO* stream, void* lptr, int size)
{
    stream._code = 0;
    stream._pos = 0;
    stream._len = size;
...
int is_open_file(IMDIO* stream, byte* filename, byte* rw)
{
    stream._code = -1;
...
void* is_read_marked_block(IMDIO* stream, byte* mark,
    int marksize, size_t* nbytes_readed)
{
    public static byte dmark[2] = { 0, 0 };
    byte* data;
    byte* buffer;
    size_t block = 0;
    size_t nsize = 0;
    ...
    buffer = (byte*)malloc(marksize + 10);

    if (buffer == null)
        return null;

    data = (byte*)malloc(256);
    block = 256;
...
typedef public struct _IBITMAPFILEHEADER
{
    uint bfType;
    uint bfSize;
    ushort bfReserved1;
...
typedef public struct _IBITMAPINFOHEADER
{
    uint biWidth;
    uint biHeight;
    ushort biBitCount;
...
//---------------------------------------------------------------------
public static int iread_bmfileheader(IMDIO* stream, IBITMAPFILEHEADER* fileheader)
{
...
//---------------------------------------------------------------------
public static int iread_bminfoheader(IMDIO* stream, IBITMAPINFOHEADER* infoheader,
    int ihsize)
{
    int data[20];
    ...
    {
        data[0] = is_igetl(stream);     /* uint  biWidth......... */
        data[1] = is_igetl(stream);     /* uint  biHeight........ */
        data[2] = is_igetw(stream);     /* ushort biPlanes........ */
        data[3] = is_igetw(stream);     /* ushort biBitCount...... */
        data[4] = is_igetl(stream);     /* uint  biCompression... */
        data[5] = is_igetl(stream);     /* uint  biSizeImage..... */
        data[6] = is_igetl(stream);     /* uint  biXPelsPerMeters */
        data[7] = is_igetl(stream);     /* uint  biYPelsPerMeters */
        data[8] = is_igetl(stream);     /* uint  biClrUsed....... */
        data[9] = is_igetl(stream);     /* uint  biClrImportant.. */

        infoheader.biWidth = data[0];
        infoheader.biHeight = data[1];
        infoheader.biBitCount = (ushort)data[3];
        infoheader.biCompression = data[4];
    }
    else
        if (ihsize == IOS2INFOHEADERSIZE)
    {
        data[0] = is_igetw(stream);     /* ushort biWidth......... */
        data[1] = is_igetw(stream);     /* ushort biHeight........ */
        data[2] = is_igetw(stream);     /* ushort biPlanes........ */
        data[3] = is_igetw(stream);     /* ushort biBitCount...... */
...
// bmp - read_bmicolors
//---------------------------------------------------------------------
public static void ibmp_read_bmicolors(int ncols, IRGB* pal, IMDIO* stream,
    int win_flag)
{
    int i;
    for (i = 0; i < ncols; i++)
...
// bmp - read_image
//---------------------------------------------------------------------
public static void ibmp_read_image(IMDIO* stream, ibitmap_t* bmp, const
    ...
    IBITMAPINFOHEADER* infoheader)
{
    ...
    for (start = 0; start < (int)infoheader.biHeight; start++)
    {
        line = infoheader.biHeight - start - 1;
        length = (int)infoheader.biWidth;
        ...
        switch (infoheader.biBitCount)
        {
            ...
            case 4:
                for (i = 0; i < length; i++)
                {
                    ...
                    if (j == 0)
                    {
                        ...
                        for (k = 0; k < 4; k++)
                        {
                            b[k * 2 + 1] = (byte)(((n & 0xFF) >> 0) & 0x0F);
                            b[k * 2 + 0] = (byte)(((n & 0xFF) >> 4) & 0x0F);
                            n = n >> 8;
                        }
                    }
                    _is_put8(bmp, i, line, b[j]);
            ...
            case 32:
                for (i = 0; i < length; i++)
                {
                    ...
                    if (bmp.bpp == 32)
                    {
                        uint cc = IRGBA_TO_A8R8G8B8(c.r, c.g, c.b, a);
...
//---------------------------------------------------------------------
public static void ibmp_read_bitfields_image(IMDIO* stream, ibitmap_t* bmp,
    IBITMAPINFOHEADER* infoheader)
{
    int i, j;
    int bpp;
    ...
    for (j = (int)infoheader.biHeight - 1; j >= 0; j--)
    {
        for (i = 0; i < (int)infoheader.biWidth; i++)
        {
...
// bmp - read_rle_image
//---------------------------------------------------------------------
public static void ibmp_read_rle_image(IMDIO* stream, ibitmap_t* bmp,
    IBITMAPINFOHEADER* infoheader)
{
    byte count, val, val0;
    byte b[8];
    int line, j, k, pos, bpp;
    ...
    for (eopicflag = 0; eopicflag == 0;)
    {
        for (eolflag = 0, pos = 0; eolflag == 0 && eopicflag == 0;)
        {
            ...
            if (pos >= (int)infoheader.biWidth) eolflag = 1;
        }
...
//---------------------------------------------------------------------
ref IBITMAPiload_bmp_stream(IMDIO * stream, IRGB * pal)
{
    ...
    if (infoheader.biCompression == IBI_BITFIELDS)
    {
        uint redMask = is_igetl(stream);
        uint grnMask = is_igetl(stream);
        uint bluMask = is_igetl(stream);
        (void)grnMask;
    ...
    /* conpublic struct a fake palette if 8-bit mode is not involved */
...
//---------------------------------------------------------------------
int isave_bmp_stream(IMDIO* stream, in IBITMAP bmp, IRGB* pal)
{
    uint c, p;
    int i, j;
    IRGB tmppal[256];
    ...
    is_iputl(stream, (int)bmp.w);  /* biWidth */
    is_iputl(stream, (int)bmp.h); /* biHeight */
    ...
    /* image data */
    for (j = (int)bmp.h - 1; j >= 0; j--)
    {
        for (i = 0; i < (int)bmp.w; i++)
        {
            c = _is_getpx(bmp, i, j);
            p = _im_color_get(fmt, c, pal);
            if (bpp == 8)
            {
                is_putc(stream, (int)c);
            }
            else
            {
        ...
        for (i = 0; i < (int)filler; i++) is_putc(stream, 0);
    }
...
//---------------------------------------------------------------------
public static uint single_tga_readn(IMDIO* stream, int bpp)
{
    uint r, g, b, a, c;
...
// tga - raw_tga_readn
//---------------------------------------------------------------------
public static void* raw_tga_readn(void* b, int w, int bpp, IMDIO* stream)
{
    byte* lptr = (byte*)b;
    uint c, n;
...
//---------------------------------------------------------------------
ref IBITMAPiload_tga_stream(IMDIO * stream, IRGB * pal)
{
    byte image_palette[256][3];
    byte id_length, palette_type, image_type, palette_entry_size;
    byte bpp, descriptor_bits;
    uint c, i, y, yc, n, x;
    ushort palette_colors;
    ...
    if (palette_type == 1)
    {
        for (i = 0; i < (uint)palette_colors; i++)
        {
            if (palette_entry_size == 16)
            {
                c = is_igetw(stream);
                image_palette[i][0] = (byte)_ipixel_scale_5[(c >> 0) & 31];
                image_palette[i][1] = (byte)_ipixel_scale_5[(c >> 5) & 31];
                image_palette[i][2] = (byte)_ipixel_scale_5[(c >> 10) & 31];
            }
    ...
    for (y = image_height; y; y--)
    {
        ...
        {
            for (x = 0; x < (uint)image_width; x += count)
            {
                count = is_getc(stream);
                if (count & 0x80)
                {
...
//---------------------------------------------------------------------
int isave_tga_stream(IMDIO* stream, in IBITMAP bmp, IRGB* pal)
{
    byte image_palette[256][3];
    uint c, a, r, g, b, p;
    int x, y, depth, n;
...
// Add new loader to load other picture formats
//---------------------------------------------------------------------
public static IPICLOADER iloader_table[256];
...
// recognize data stream and load picture
//---------------------------------------------------------------------
ref IBITMAPiload_picture(IMDIO * stream, IRGB * pal)
{
    byte firstbyte;
    int ch;
...
// ORIGINAL OPERATION
//=====================================================================
public static IRGB _idefault_pal[256];
...
void ipic_write_pal(in void* _pal)
{
    memcpy(_idefault_pal, _pal, 256 * 4);
}

ref IBITMAPipic_convert(ref IBITMAP src, int dfmt, IRGB* pal)
{
	in IBITMAP  bmp;
int sfmt;
int w, h;
...
public struct ref IBITMAP ipic_load_file(byte * file, int pos, IRGB * pal)
{
    IMDIO stream;
	in IBITMAP bmp;
...
ref IBITMAPipic_load_mem(const void* ptr, int size, IRGB* pal)
{
	IMDIO stream;
	in IBITMAP bmp;
IRGB* p = pal == null ? _ipaletted : pal;
...
//---------------------------------------------------------------------
public static void _igif_clear_table(IGIFDESC* gif)
{
...
//---------------------------------------------------------------------
public static void _igif_get_code(IGIFDESC* gif)
{
...
//---------------------------------------------------------------------
public static void _igif_get_string(IGIFDESC* gif, int num)
{
    byte* string;
    int i;
    string = gif.string;
...
//---------------------------------------------------------------------
public static void _igif_output_string(IGIFDESC* gif)
{
    byte* string, **line, *ptr, mask;
...
//---------------------------------------------------------------------
// ipic_gif_open - open gif and load header.load gif header, allocate bitmap if customer is false
//---------------------------------------------------------------------
int ipic_gif_open(IGIFDESC* gif, IMDIO* stream, IRGB* pal, int customer)
{
    ...
    gif.background = (byte)is_getc(stream);
    gif.aspect = (byte)is_getc(stream);
...
//---------------------------------------------------------------------
public static int ipic_gif_read_image_desc(IGIFDESC* gif)
{
    byte* string;
    IMDIO* stream;
...
int ipic_gif_read_extension(IGIFDESC* gif, int firstcmd)
{
    ...
    if (control == 0xf9)
    {
        ...
        gif.bitmap.mask = (byte)gif.transparent;
...
//---------------------------------------------------------------------
// ipic_gif_read_frame - read single frame,returns delay time, below zero for eof or error
//---------------------------------------------------------------------
...
//---------------------------------------------------------------------
ref IBITMAPiload_gif_stream(IMDIO * stream, IRGB * pal)
{
	in IBITMAP bmp;
    IGIFDESC* gif;
...
//---------------------------------------------------------------------
// open gif stream for saving,write gif header, allocate bitmap if IGIF_CUSTOMERBMP flag is 0
//---------------------------------------------------------------------
int ipic_gif_wopen(IGIFDESC* gif, IMDIO* stream, IRGB* pal,
    int w, int h, int background, int aspect, int flags, int hotxy)
{
    int i;
...
//---------------------------------------------------------------------
public static void _igif_write_code(IGIFDESC* gif, int code, int bit_size)
{
    if (code < 0)
    {
        gif.entire &= (1 << gif.bit_pos) - 1;
        gif.string[gif.data_len] = (byte)gif.entire;
        gif.entire >>= 8;
        gif.data_len++;
    ...
    while (gif.bit_pos >= 8)
    {
        gif.string[gif.data_len] = (byte)(gif.entire & 255);
        gif.entire >>= 8;
        gif.bit_pos -= 8;
        gif.data_len++;
        if (gif.data_len >= 252)
...
//---------------------------------------------------------------------
public static int _igif_get_next_color(IGIFDESC* gif)
{
    int x = gif.x;
    int y = gif.y;
    int c;
...
//---------------------------------------------------------------------
public static int ipic_gif_write_image_desc(IGIFDESC* gif, int palsize)
{
    IMDIO* stream;
...
//---------------------------------------------------------------------
// ipic_gif_write_frame - write frame into gif, write frame into gif stream
//---------------------------------------------------------------------
...
// save gif picture to stream
//---------------------------------------------------------------------
int isave_gif_stream(IMDIO* stream, in IBITMAP bmp, IRGB* pal)
{
    IGIFDESC* gif;
    int flags;
    int hotxy;
